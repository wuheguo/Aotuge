...
public class CalendarCard extends View {
		...
		for (int j = 0; j < TOTAL_ROW; j++) {
			...
			for (int i = 0; i < TOTAL_COL; i++) {
				...
				if (position >= firstDayWeek
					...
					// 今天
					if(AotugeApplication.getInstance().mCurdate == null)
					{
					...
					else
					{
						  ...
						  CustomDate date1 = CustomDate.modifiDayForObject(mShowDate, day);
						  CustomDate date2 =AotugeApplication.getInstance().mCurdate;
			...
			for (int i = 0; i < TOTAL_COL; i++) {
				...
				if (position >= firstDayWeek
					...
					else
					{
							...
							if (isCurrentMonth && day == AotugeApplication.getInstance().mCurdate.day  ) {
								CustomDate date = CustomDate.modifiDayForObject(mShowDate, day);
								rows[j].cells[i] = new Cell(date, State.TODAY, i, j);
							}
					}
	...
	private void measureClickCell(int col, int row) {
		...
		if (rows[row] != null) {
			...
			date.week = col;
			AotugeApplication.getInstance().mCurdate = date;
